...
_LAUNCHER_SCRIPT_STEM = 'tableA11'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'tableA11.txt'
...
# -----------------------------
# 2. Construct lagged municipality-year mean outcome for women
# -----------------------------
lag = df.loc[df["female"] == 1].copy()

lag["mean_gewinn_norm"] = (
    lag.groupby(["gkz", "jahr"])["gewinn_norm"].transform("mean")
)

lag = (
    lag[["gkz", "jahr", "mean_gewinn_norm"]]
    .drop_duplicates()
    .rename(columns={"mean_gewinn_norm": "lag_mean_gewinn_norm"})
)

lag["jahr"] = pd.to_numeric(lag["jahr"], errors="coerce") + 5
lag = lag.loc[lag["jahr"] != 2021].copy()

df = df.merge(lag, on=["gkz", "jahr"], how="left")
...
def fit_rd_spec(data, h, poly="Linear"):
    ...
    if poly == "Linear":
        cols = ["female_mayor", "lag_mean_gewinn_norm", "margin_1", "inter_1"]
    elif poly == "Quadratic":
        cols = ["female_mayor", "lag_mean_gewinn_norm", "margin_1", "inter_1", "margin_2", "inter_2"]
    else:
    ...
    return {
        "coef_female_mayor": float(result.params["female_mayor"]),
        "se_female_mayor": float(result.bse["female_mayor"]),
        "pval_female_mayor": float(result.pvalues["female_mayor"]),
        "coef_lag": float(result.params["lag_mean_gewinn_norm"]),
        "se_lag": float(result.bse["lag_mean_gewinn_norm"]),
        "pval_lag": float(result.pvalues["lag_mean_gewinn_norm"]),
        "N": int(len(temp)),
...
# -----------------------------
specs = [
    {"bw_type": "CCT",   "h": 20.10, "poly": "Linear"},
    {"bw_type": "CCT/2", "h": 10.05, "poly": "Linear"},
    {"bw_type": "2CCT",  "h": 40.20, "poly": "Linear"},
    {"bw_type": "IK",    "h": 35.59, "poly": "Linear"},
    {"bw_type": "CCT",   "h": 19.93, "poly": "Quadratic"},
]
...
# -----------------------------
# 7. Print exact text-style output
# -----------------------------
line = "-" * 92
...
print(
    f"{'female_mayor':<22}"
    f"{f'{results[0]['coef_female_mayor']:.3f}{stars(results[0]['pval_female_mayor'])}':>14}"
    f"{f'{results[1]['coef_female_mayor']:.3f}{stars(results[1]['pval_female_mayor'])}':>16}"
    f"{f'{results[2]['coef_female_mayor']:.3f}{stars(results[2]['pval_female_mayor'])}':>16}"
    f"{f'{results[3]['coef_female_mayor']:.3f}{stars(results[3]['pval_female_mayor'])}':>16}"
    f"{f'{results[4]['coef_female_mayor']:.3f}{stars(results[4]['pval_female_mayor'])}':>16}"
)

print(
    f"{'':<22}"
    f"{f'({results[0]['se_female_mayor']:.3f})':>14}"
    f"{f'({results[1]['se_female_mayor']:.3f})':>16}"
    f"{f'({results[2]['se_female_mayor']:.3f})':>16}"
    f"{f'({results[3]['se_female_mayor']:.3f})':>16}"
    f"{f'({results[4]['se_female_mayor']:.3f})':>16}"
)

print(
    f"{'lag_mean_g~m':<22}"
    f"{f'{results[0]['coef_lag']:.3f}{stars(results[0]['pval_lag'])}':>14}"
    f"{f'{results[1]['coef_lag']:.3f}{stars(results[1]['pval_lag'])}':>16}"
    f"{f'{results[2]['coef_lag']:.3f}{stars(results[2]['pval_lag'])}':>16}"
    f"{f'{results[3]['coef_lag']:.3f}{stars(results[3]['pval_lag'])}':>16}"
    f"{f'{results[4]['coef_lag']:.3f}{stars(results[4]['pval_lag'])}':>16}"
)

print(
    f"{'':<22}"
    f"{f'({results[0]['se_lag']:.3f})':>14}"
    f"{f'({results[1]['se_lag']:.3f})':>16}"
    f"{f'({results[2]['se_lag']:.3f})':>16}"
    f"{f'({results[3]['se_lag']:.3f})':>16}"
    f"{f'({results[4]['se_lag']:.3f})':>16}"
)
...
# 8. Save the table
# -----------------------------
with open("TableA11.txt", "w", encoding="utf-8") as f:
    f.write(line + "\n")
