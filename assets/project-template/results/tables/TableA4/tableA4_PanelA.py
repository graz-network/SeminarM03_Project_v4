...
_LAUNCHER_SCRIPT_STEM = 'tableA4_PanelA'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'TableA4_PanelA.txt'
...
# -----------------------------
# 3. Variables used in Table A4 Panel A
# -----------------------------
variables = [
    "log_bevoelkerung",
    "log_flaeche",
...
labels = {
    "log_bevoelkerung": "Log(population)",
    "log_flaeche": "Log(land area)",
    "log_debt_pc": "Log(debt p.c.)",
    "log_tottaxrev_pc": "Log(tax revenues p.c.)",
    "log_gemeinde_beschaef_pc": "Log(public employment p.c.)",
...
for var in variables:
    ...
    rows.append({
        "Variable": labels[var],
        "Female mayor": round(female_mayor.mean(), 3),
        "Male mayor": round(male_mayor.mean(), 3),
        "Diff.": f"{diff:.3f}{stars(ttest.pvalue)}",
...
# 6. Add N row
# -----------------------------
n_obs = int(df[[group_var]].dropna().shape[0])
...
table_final = pd.concat([table_final, n_row], ignore_index=True)
...
# 8. Save the table
# -----------------------------
table_final.to_string("TableA4_PanelA.txt", index=False)