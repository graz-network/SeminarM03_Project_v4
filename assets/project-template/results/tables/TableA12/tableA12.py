...
_LAUNCHER_SCRIPT_STEM = 'tableA12'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'tableA12.txt'
...
# -----------------------------
# 3. Build predicted_rank_change
# -----------------------------
mask = (
    pd.to_numeric(work["gewinn_norm"], errors="coerce").notna()
    & pd.to_numeric(work["listenplatz_norm"], errors="coerce").notna()
)

reg = work.loc[mask].copy()

y = pd.to_numeric(reg["gewinn_norm"], errors="coerce").astype(float)
X = pd.DataFrame(
    {
        "listenplatz_norm": pd.to_numeric(
            reg["listenplatz_norm"], errors="coerce"
        ).astype(float)
    }
)
...
pred_X = pd.DataFrame(
    {
        "listenplatz_norm": pd.to_numeric(
            work["listenplatz_norm"], errors="coerce"
        ).astype(float)
    }
)
pred_X = sm.add_constant(pred_X, has_constant="add")

work["predicted_rank_change"] = first_stage.predict(pred_X)
...
def fit_rd_spec(data, h, poly="Linear"):
    ...
    temp = temp.dropna(
        subset=[depvar, cluster_col, "gkz", "jahr"] + cols
    ).copy()
...
# -----------------------------
specs = [
    {"bw_type": "CCT",   "h": 14.52, "poly": "Linear"},
    {"bw_type": "CCT/2", "h": 7.26,  "poly": "Linear"},
    {"bw_type": "2CCT",  "h": 29.04, "poly": "Linear"},
    {"bw_type": "IK",    "h": 17.17, "poly": "Linear"},
    {"bw_type": "CCT",   "h": 26.12, "poly": "Quadratic"},
]

results = []
for spec in specs:
    res = fit_rd_spec(work, h=spec["h"], poly=spec["poly"])
...
# 8. Save the table
# -----------------------------
with open("TableA12.txt", "w", encoding="utf-8") as f:
    f.write(table_text)