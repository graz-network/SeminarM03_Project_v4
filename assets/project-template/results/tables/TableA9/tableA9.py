...
_LAUNCHER_SCRIPT_STEM = 'tableA9'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'tableA9.txt'
...
# 1. Load dataset
# --------------------------------------------------
df = pd.read_stata("../../../data/raw/dataset_with_rank_improvments_next_election.dta")

depvar = "gewinn_norm"
treatment = "female_mayor"
running = "margin_1"
cluster = "gkz"
election_id = "gkz_jahr"
...
def run_rd(data, h, poly="Linear"):
    temp = data[np.abs(pd.to_numeric(data[running], errors="coerce")) < h].copy()

    if poly == "Linear":
        cols = [treatment, "margin_1", "inter_1"]
    else:
        cols = [treatment, "margin_1", "inter_1", "margin_2", "inter_2"]

    temp = temp.dropna(subset=[depvar, cluster] + cols)

    y = pd.to_numeric(temp[depvar], errors="coerce")
    X = temp[cols].apply(pd.to_numeric, errors="coerce")
    X = sm.add_constant(X)

    w = triangular_weights(temp[running].astype(float).values, h)

    model = sm.WLS(y, X, weights=w)
    res = model.fit(cov_type="cluster", cov_kwds={"groups": temp[cluster]})

    elections = temp[election_id].nunique()

    return {
        "coef": float(res.params[treatment]),
        "se": float(res.bse[treatment]),
        "p": float(res.pvalues[treatment]),
        "N": len(temp),
        "Elections": elections,
        "Municipalities": temp[cluster].nunique(),
        "Mean (SD)": f"{temp[depvar].mean():.2f} ({temp[depvar].std():.2f})",
    }

# --------------------------------------------------
# 3. Specifications (bandwidths from Stata output)
# --------------------------------------------------
specs = [
    {"bw": "CCT",   "h": 16.99, "poly": "Linear"},
    {"bw": "CCT/2", "h": 8.50,  "poly": "Linear"},
    {"bw": "2CCT",  "h": 33.98, "poly": "Linear"},
    {"bw": "IK",    "h": 14.40, "poly": "Linear"},
    {"bw": "CCT",   "h": 20.27, "poly": "Quadratic"},
]

results = []
for s in specs:
    r = run_rd(df, s["h"], s["poly"])
    r["bw_type"] = s["bw"]
    r["bw_size"] = s["h"]
    r["poly"] = s["poly"]
    results.append(r)
...
lines.append(
    f"{'female_mayor':<22}"
    f"{f'{results[0]['coef']:.3f}{stars(results[0]['p'])}':>14}"
    f"{f'{results[1]['coef']:.3f}{stars(results[1]['p'])}':>16}"
    f"{f'{results[2]['coef']:.3f}{stars(results[2]['p'])}':>16}"
    f"{f'{results[3]['coef']:.3f}{stars(results[3]['p'])}':>16}"
    f"{f'{results[4]['coef']:.3f}{stars(results[4]['p'])}':>16}"
)
