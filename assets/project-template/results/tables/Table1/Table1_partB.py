...
_LAUNCHER_SCRIPT_STEM = 'Table1_partB'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'Table1_partB.txt'
...
# 4. Check columns
# -----------------------------
missing = [col for col in variables + ["female"] if col not in df.columns]
if missing:
...
table_final = pd.concat([table, final_row], ignore_index=True)
...
# 8. Save the table
# -----------------------------
table_final.to_string("Table1_PartB.txt", index=False)