...
_LAUNCHER_SCRIPT_STEM = 'table6_PanelB'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'table6_PanelB.txt'
...
# -----------------------------
# 2. Variable setup for Table 6 Panel B
# -----------------------------
depvar = "predicted_rank_change"
target_col = "gewinn_norm"
...
# -----------------------------
# 3. Build predicted rank change for men
# -----------------------------
pred_sample = df[(df["rdd_sample"] == 1) & (df["female"] == 0)].copy()

pred_sample = pred_sample.dropna(
    subset=[target_col] + prediction_controls
).copy()

y_pred_model = pred_sample[target_col].astype(float)
X_pred_model = pred_sample[prediction_controls].astype(float)
X_pred_model = sm.add_constant(X_pred_model, has_constant="add")

pred_model = sm.OLS(y_pred_model, X_pred_model).fit()

df_men = df[(df["rdd_sample"] == 1) & (df["female"] == 0)].copy()
df_men = df_men.dropna(
    subset=prediction_controls + [
        treatment_col,
        running_col,
        cluster_col,
        election_id_col,
...
X_all = df_men[prediction_controls].astype(float)
X_all = sm.add_constant(X_all, has_constant="add")
df_men[depvar] = pred_model.predict(X_all)
...
# -----------------------------
# 5. Table 6 Panel B specifications
# -----------------------------
specs = [
    {"col": "(1)", "bw_type": "CCT",   "h": 19.20, "poly": "Linear"},
    {"col": "(2)", "bw_type": "CCT/2", "h": 9.60,  "poly": "Linear"},
    {"col": "(3)", "bw_type": "2CCT",  "h": 38.40, "poly": "Linear"},
    {"col": "(4)", "bw_type": "IK",    "h": 15.20, "poly": "Linear"},
    {"col": "(5)", "bw_type": "CCT",   "h": 24.61, "poly": "Quadratic"},
]
...
results = []
for spec in specs:
    res = fit_rd_spec(df_men, h=spec["h"], poly=spec["poly"])
...
# 9. Save the table
# -----------------------------
table_final.to_string("Table6_PanelB.txt", index=False)