...
_LAUNCHER_SCRIPT_STEM = 'table6_PanelA'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'table6_PanelA.txt'
...
# -----------------------------
# 3. Build predicted rank change for women
# -----------------------------
pred_sample = df[(df["rdd_sample"] == 1) & (df["female"] == 1)].copy()
...
df_women = df[(df["rdd_sample"] == 1) & (df["female"] == 1)].copy()
...
X_all = df_women[prediction_controls].astype(float)
X_all = sm.add_constant(X_all, has_constant="add")

df_women[depvar] = pred_model.predict(X_all)
...
def fit_rd_spec(data, h, poly="Linear"):
    ...
    temp = temp.dropna(
        subset=[depvar, cluster_col, election_id_col] + cols
    ).copy()
...
# -----------------------------
specs = [
    {"bw_type": "CCT", "h": 18.37, "poly": "Linear"},
    {"bw_type": "CCT/2", "h": 9.19, "poly": "Linear"},
    {"bw_type": "2CCT", "h": 36.74, "poly": "Linear"},
    {"bw_type": "IK", "h": 21.94, "poly": "Linear"},
    {"bw_type": "CCT", "h": 21.52, "poly": "Quadratic"},
]
...
for spec in specs:
    res = fit_rd_spec(df_women, spec["h"], spec["poly"])
...
# 9. Save the table
# -----------------------------
table_final.to_string("Table6_PanelA.txt", index=False)