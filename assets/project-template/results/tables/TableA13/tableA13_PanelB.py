...
_LAUNCHER_SCRIPT_STEM = 'tableA13_PanelB'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'tableA13_PanelB.txt'
...
# 2. Restrict sample
# -----------------------------
work = df[(df["rdd_sample"] == 1) & (df["female"] == 1)].copy()

# -----------------------------
# 3. Dependent variables and fixed bandwidths
# -----------------------------
specs = [
    {"depvar": "architect",             "label": "(1)", "bw_type": "CCT", "h": 19.44},
    {"depvar": "businessmanwoman",      "label": "(2)", "bw_type": "CCT", "h": 19.75},
    {"depvar": "engineer",              "label": "(3)", "bw_type": "CCT", "h": 17.76},
    {"depvar": "lawyer",                "label": "(4)", "bw_type": "CCT", "h": 13.97},
    {"depvar": "civil_administration",  "label": "(5)", "bw_type": "CCT", "h": 17.92},
    {"depvar": "teacher",               "label": "(6)", "bw_type": "CCT", "h": 24.03},
]
...
def fit_rd_spec(data, depvar, h):
    ...
    return {
        "coef": float(result.params["female_mayor"]),
        "se": float(result.bse["female_mayor"]),
        "pval": float(result.pvalues["female_mayor"]),
        "bw_type": "CCT",
        "bw_size": h,
        "poly": "Linear",
        "N": int(len(temp)),
...
# -----------------------------
# 5. Estimate one model per depvar
# -----------------------------
results = []
for spec in specs:
    res = fit_rd_spec(work, depvar=spec["depvar"], h=spec["h"])
    results.append(res)

# -----------------------------
# 6. Build exact text output
# -----------------------------
line = "-" * 108
lines = []

lines.append(line)
lines.append(f"{'':<22}{'(1)':>14}{'(2)':>16}{'(3)':>16}{'(4)':>16}{'(5)':>16}{'(6)':>16}")
lines.append(line)
...
lines.append(
    ...
    f"{f'{results[5]['coef']:.3f}{stars(results[5]['pval'])}':>16}"
)
...
# 7. Save the table
# -----------------------------
with open("TableA13_PanelB.txt", "w", encoding="utf-8") as f:
    f.write(table_text)