...
_LAUNCHER_SCRIPT_STEM = 'table2'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'table2.txt'
...
# -----------------------------
# 2. Variable setup for Table 2
# -----------------------------
# From the paper:
# - outcome = normalized rank improvement of a female council candidate
# - sample = female candidates in mixed-gender mayor-race municipalities
# - treatment = female mayor dummy
# - cluster = municipality
#
# Based on your column list, these are the most plausible mappings.
outcome_col = "gewinn_norm"
female_candidate_col = "female"
sample_col = "rdd_sample"
treatment_col = "female_mayor"
municipality_id_col = "gkz"
election_id_col = "gkz_jahr"

# The true running variable should be the female candidate's margin of victory
# in the last mixed-gender mayor election. In your data, that is most likely
# one of these two columns.
running_candidates = ["margin_1", "margin_2"]

# Published Table 2 values used for auto-selection diagnostics
target_bw_1 = 20.10
target_n_1 = 2878
target_mean_1 = 0.41
target_sd_1 = 10.95
...
# -----------------------------
# 4. Restrict to the Table 2 base sample
# -----------------------------
# Paper note:
# female candidates only + mixed-gender mayor-race sample
base = df.copy()
base = base[base[female_candidate_col] == 1]
base = base[base[sample_col] == 1]

# -----------------------------
# 5. Auto-pick the correct running variable
# -----------------------------
# We choose the candidate that best matches the published stats for col (1):
# N = 2878, mean = 0.41, sd = 10.95 within h = 20.10.
diagnostics = []

for run_col in available_running:
    temp = base.dropna(subset=[outcome_col, treatment_col, run_col, municipality_id_col, election_id_col]).copy()
    ...
    score = (
        ...
        + 100 * abs(mean_val - target_mean_1)
        + 20 * abs(sd_val - target_sd_1)
    )
...
def fit_sharp_rd(data, y_col, x_col, cluster_col, h, p=1):
    ...
    # Local polynomial with full interaction by side of cutoff
    # p = 1: y = a + tau*D + b1*x + c1*D*x + e
    # p = 2: add x^2 and D*x^2
    X = pd.DataFrame({
        "const": 1.0,
        "female_mayor": d,
        "x": x,
...
# -----------------------------
# 7. Table 2 specifications from the paper
# -----------------------------
specs = [
    {"col": "(1)", "bw_type": "CCT",   "h": 20.10, "p": 1, "poly": "Linear"},
    {"col": "(2)", "bw_type": "CCT/2", "h": 10.05, "p": 1, "poly": "Linear"},
    {"col": "(3)", "bw_type": "2CCT",  "h": 40.20, "p": 1, "poly": "Linear"},
    {"col": "(4)", "bw_type": "IK",    "h": 35.59, "p": 1, "poly": "Linear"},
    {"col": "(5)", "bw_type": "CCT",   "h": 19.93, "p": 2, "poly": "Quadratic"},
]
...
# 11. Save the table
# -----------------------------
table_final.to_string("Table2.txt", index=False)
