...
_LAUNCHER_SCRIPT_STEM = 'table7'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'table7.txt'
...
# -----------------------------
# 3. Keep one observation per election
# -----------------------------
base = (
...
# Keep RD sample only
base = base[base["rdd_sample"] == 1].copy()
...
def fit_rd_spec(data, h, poly="Linear"):
    ...
    elections = temp[election_id_col].nunique() if election_id_col in temp.columns else len(temp)
...
# -----------------------------
# 5. Table 7 specifications
# -----------------------------
specs = [
    {"col": "(1)", "bw_type": "CCT",   "h": 17.41, "poly": "Linear"},
    {"col": "(2)", "bw_type": "CCT/2", "h": 8.70,  "poly": "Linear"},
    {"col": "(3)", "bw_type": "2CCT",  "h": 34.82, "poly": "Linear"},
    {"col": "(4)", "bw_type": "IK",    "h": 35.40, "poly": "Linear"},
    {"col": "(5)", "bw_type": "CCT",   "h": 29.32, "poly": "Quadratic"},
]
...
# 9. Save the table
# -----------------------------
table_final.to_string("Table7.txt", index=False)