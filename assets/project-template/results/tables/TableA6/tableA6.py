...
_LAUNCHER_SCRIPT_STEM = 'tableA6'
_LAUNCHER_EXPECTED_PRIMARY_NAME = 'tableA6.txt'
...
# -----------------------------
# 4. Function to print one table
# -----------------------------
def print_one_table(data):
    x = pd.to_numeric(data["female_mayor"], errors="coerce").dropna()
    y = pd.to_numeric(data["male_mayor"], errors="coerce").dropna()
    ...
    mu_female = x.mean()
    mu_male = y.mean()
    diff = mu_female - mu_male
    se_level = x.std(ddof=1) / np.sqrt(n) if n > 1 else np.nan
    se_diff = (x - y).std(ddof=1) / np.sqrt(n) if n > 1 else np.nan

    line = "-" * 67

    print(line)
    print(f"{'':<18}{'mu':>12}{'se':>16}{'p':>13}{'obs':>12}")
    print(line)
    print(f"{'Female':<18}{mu_female:>12.3f}{f'({se_level:.3f})':>16}{'.':>13}{'.':>12}")
    print(f"{'Male':<18}{mu_male:>12.3f}{f'({se_level:.3f})':>16}{'.':>13}{'.':>12}")
    print(f"{'Diff':<18}{f'{diff:.3f}{stars(test.pvalue)}':>12}{f'({se_diff:.3f})':>16}{test.pvalue:>13.3f}{n:>12}")
    print(line)

# -----------------------------
# 5. Full RD sample
# -----------------------------
print("\nMargin window: 100% of RD sample")
print_one_table(df)

# -----------------------------
# 6. Elections with |margin_1| <= 25
# -----------------------------
df_25 = df[pd.to_numeric(df["margin_1"], errors="coerce").abs() <= 25].copy()
print("\nMargin window: 25% (|margin_1| ≤ 25)")
print_one_table(df_25)

# -----------------------------
# 7. Elections with |margin_1| <= 10
# -----------------------------
df_10 = df[pd.to_numeric(df["margin_1"], errors="coerce").abs() <= 10].copy()
print("\nMargin window: 10% (|margin_1| ≤ 10)")
print_one_table(df_10)

# -----------------------------
# 8. Save the tables
# -----------------------------
def save_one_table(data, output_path):
    x = pd.to_numeric(data["female_mayor"], errors="coerce").dropna()
    y = pd.to_numeric(data["male_mayor"], errors="coerce").dropna()
    n = min(len(x), len(y))
    x = x.iloc[:n]
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(line + "\n")
        f.write(f"{'':<18}{'mu':>12}{'se':>16}{'p':>13}{'obs':>12}\n")
        f.write(line + "\n")
        f.write(f"{'Female':<18}{mu_female:>12.3f}{f'({se_level:.3f})':>16}{'.':>13}{'.':>12}\n")
        f.write(f"{'Male':<18}{mu_male:>12.3f}{f'({se_level:.3f})':>16}{'.':>13}{'.':>12}\n")
        f.write(f"{'Diff':<18}{f'{diff:.3f}{stars(test.pvalue)}':>12}{f'({se_diff:.3f})':>16}{test.pvalue:>13.3f}{n:>12}\n")
        f.write(line + "\n")

save_one_table(df, "TableA6_100percent.txt")
save_one_table(df_25, "TableA6_25percent.txt")
save_one_table(df_10, "TableA6_10percent.txt")